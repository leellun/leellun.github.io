...
cfssl gencert -ca=/usr/local/k8s-install/cert/ca.pem -ca-key=/usr/local/k8s-install/cert/ca-key.pem  --config=/usr/local/k8s-install/cert/ca-config.json -profile=kubernetes   kube-proxy-csr.json | cfssljson -bare kube-proxy

cp -r {kube-proxy.pem,kube-proxy-key.pem} /opt/kubernetes/ssl
```
...
cfssl gencert -ca=/usr/local/k8s-install/cert/ca.pem -ca-key=/usr/local/k8s-install/cert/ca-key.pem -config=/usr/local/k8s-install/cert/ca-config.json -profile=kubernetes etcd-csr.json | cfssljson -bare etcd

cp {/usr/local/k8s-install/cert/ca.pem,etcd-key.pem,etcd.pem} /opt/etcd/ssl/
```
...
## 3.7 font-proxy 生成apiserver聚合证书
...
-config=/usr/local/k8s-install/cert/ca-config.json   \
